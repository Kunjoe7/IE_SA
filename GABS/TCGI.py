...
df = pd.read_csv('/project/lwang/xqin5/SAcompare/GABS/gabs.csv')
data = df
...
print(f"Survival time min: {data.duration.min()}, max: {data.duration.max()}")




# Sort the DataFrame by the specified column
data_sorted = data.sort_values(by='duration').reset_index(drop=True)
...
data['interval_number'] = data['duration'].apply(
    lambda st: data_sorted[data_sorted['duration'] == st]['interval_number'].iloc[0]
)
...
data_sorted["mask_vector"] = data_sorted.apply(lambda x: mask_vector(x.interval_number, x.event), axis=1)




train_indices, test_indices = train_test_split(
        range(len(data_sorted)), stratify=data_sorted['event'], random_state=1, test_size=0.25
    )

X = data_sorted.drop(['duration', 'event', 'interval_number', 'label_vector', 'mask_vector'], axis=1)
Y = np.array(data_sorted['label_vector'].values.tolist())
W = np.array(data_sorted['mask_vector'].values.tolist())
events = data_sorted['event'].values
...
# if trainer.attribution_matrix is None:
#     raise ValueError("Attribution matrix was not computed during training.")

# # 转换为 numpy 数组
# attribution_matrix = trainer.attribution_matrix.cpu().numpy()  # shape=(batch_size, n_features)
# feature_columns = X_columns

# #==============================================================================#
# #                 1) 计算并保存 Global Feature Importance 到 CSV               #
# #==============================================================================#

# # 先计算每个特征的“平均归因”作为全局重要性
# global_importances = attribution_matrix.mean(axis=0)  # shape = (n_features,)
# # 按绝对值从大到小排序，得到排序后特征的索引
# sorted_indices_global_full = np.argsort(-np.abs(global_importances))

# # 1.1) 将排序结果写入 CSV（从最重要到最不重要），并保留正负号
# csv_global = "global_feature_importance.csv"  # 当前目录下
# with open(csv_global, "w", newline='', encoding='utf-8') as f:
#     writer = csv.writer(f)
#     writer.writerow(["Feature", "Global_Importance"])
#     for idx in sorted_indices_global_full:
#         writer.writerow([feature_columns[idx], global_importances[idx]])

# print(f"All global feature importance saved to {csv_global}")

# # 1.2) 只取前 top_n 个来画图
# top_n = 10
# sorted_indices_global = sorted_indices_global_full[:top_n]
# top_global_attributions = global_importances[sorted_indices_global]
# top_global_columns = [feature_columns[i] for i in sorted_indices_global]

# # 绘制水平条形图 (Global)
# plt.figure(figsize=(10, 8))
# # 根据正负值设置不同颜色
# plt.barh(top_global_columns,
#          top_global_attributions,
#          color=['#FF0051' if g > 0 else '#0051FF' for g in top_global_attributions])

# # 在 bar 上添加数值标签（保留 6 位小数）
# for index, value in enumerate(top_global_attributions):
#     plt.text(value, index, f'{value:.6f}', va='center', fontsize=8, color='black')

# plt.xlabel("Global Feature Importance Value")
# plt.title("Top 10 Global Feature Importance", fontsize=14, fontweight='bold')
# plt.tight_layout()

# output_file_global = "top_10_global_feature_importance.png"
# plt.savefig(output_file_global, bbox_inches='tight')
# plt.show()
# print(f"Global feature importance plot saved to {output_file_global}")

# #==============================================================================#
# #                  2) 计算并保存 Local Feature Importance 到 CSV               #
# #==============================================================================#
...
# local_gradients = X_sample.grad.cpu().numpy().flatten()  # shape = (n_features,)

# # 按绝对值从大到小排序
# sorted_indices_local_full = np.argsort(-np.abs(local_gradients))

# # 2.1) 将所有特征的 Local Importance 写入 CSV
# csv_local = f"local_feature_importance_sample_{sample_idx}.csv"
# with open(csv_local, "w", newline='', encoding='utf-8') as f:
#     writer = csv.writer(f)
#     writer.writerow(["Feature", "Local_Importance"])
#     for idx in sorted_indices_local_full:
#         writer.writerow([feature_columns[idx], local_gradients[idx]])

# print(f"All local feature importance saved to {csv_local}")

# # 2.2) 只取前 top_n 个用来作图
# top_local_gradients = local_gradients[sorted_indices_local_full[:top_n]]
# top_local_columns = [feature_columns[i] for i in sorted_indices_local_full[:top_n]]

# plt.figure(figsize=(10, 8))
# plt.barh(top_local_columns,
#          top_local_gradients,
#          color=['#FF0051' if g > 0 else '#0051FF' for g in top_local_gradients])

# # 在 bar 上添加数值标签（保留 6 位小数）
# for index, value in enumerate(top_local_gradients):
#     plt.text(value, index, f'{value:.6f}', va='center', fontsize=8, color='black')

# plt.xlabel("Local Feature Importance Value")
# plt.title(f"Top 10 Local Feature Importance (Sample {sample_idx})", fontsize=14, fontweight='bold')
# plt.tight_layout()

# output_file_local = f"top_10_local_feature_importance_sample_{sample_idx}.png"
# plt.savefig(output_file_local, bbox_inches='tight')
# plt.show()
# print(f"Local feature importance plot saved to {output_file_local}")